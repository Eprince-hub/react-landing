...
&:after {
	...
	height: 180%;
	position: absolute;
	background-image: url(${handLeft} ),url(${handLeftRepeat}),url(${handRight}),url(${handRightRepeat}),url(${computers}),url(${cables});
 background-position: 40%,0 0,50%,100% 0,50%,0;
 background-repeat: no-repeat;
 background-size: auto 100%,0 100%,auto 100%,calc(50vw - 1000px) 100%,auto 100%,50vw 100%;
 display: block;
/* left: 50%; */
/* margin-left: -50vw; */
right: 1rem;
		...
		<section css={heroPageStyle}>
			...
			<main>
				...
				<div>
					...
					<p>Design, develop, and deploy WordPress sites from the comfort of your local machine. DevKinsta is free forever, and available for macOS, Windows, and Ubuntu. Used by more than 12,000 developers, web designers, and freelancers.</p>
